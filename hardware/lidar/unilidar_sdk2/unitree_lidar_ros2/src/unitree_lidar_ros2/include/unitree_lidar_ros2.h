...
public:
    ...
    // Unitree Lidar Reader - Made public for signal handler access
...
UnitreeLidarSDKNode::~UnitreeLidarSDKNode()
{
    if (lsdk_)
    {
        std::cout << "[INFO] Destructor: Stopping lidar rotation..." << std::endl;
        lsdk_->stopLidarRotation();
    }
}

void UnitreeLidarSDKNode::shutdown_callback()
{
    if (lsdk_)
    {
        std::cout << "[INFO] Shutdown callback: Stopping lidar rotation..." << std::endl;
        lsdk_->stopLidarRotation();
    }
}
