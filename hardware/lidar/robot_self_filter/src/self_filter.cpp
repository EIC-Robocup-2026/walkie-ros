...
namespace robot_self_filter
{
  ...
  public:
    SelfFilterNode()
        : Node("self_filter")
    {
      // NOTE: use_sim_time is supplied by the launch file (false on real
      // hardware, true in simulation). Do NOT force it true here: on the real
      // robot there is no /clock publisher, so forcing sim time freezes the
      // node clock at 0. That breaks the timed TF lookups in
      // SelfMask::assumeFrame(), the sensor-frame lookup falls back to (0,0,0)
      // (the robot base), and every point gets classified as SHADOW and
      // dropped -- i.e. an empty filtered cloud. Respect whatever the launch
      // passed instead.
