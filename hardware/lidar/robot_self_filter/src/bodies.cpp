...
void ConvexMesh::updateInternalData()
{
  ...
  // Inflate the quick-reject bounding box by the same scale/padding so it never
  // rejects a point that the (now larger) hull would actually contain.
  ...
  // Fold scale + padding into each plane's offset. A plane is (n, d) with outward
  // unit normal n and inside <=> n.p + d <= 0. Scaling about m_meshCenter by s and
  // pushing the plane out by `pad` metres gives:
  //   d_eff = s*d + (s-1)*(n . meshCenter) - pad
  m_paddedPlanes.resize(m_planes.size());
  for (size_t i = 0; i < m_planes.size(); ++i)
  {
    const Eigen::Vector4d &pl = m_planes[i];
    double nDotCenter = pl(0)*m_meshCenter.x() + pl(1)*m_meshCenter.y() + pl(2)*m_meshCenter.z();
    double d_eff = m_scale * pl(3) + (m_scale - 1.0) * nDotCenter - m_padding;
    m_paddedPlanes[i] = Eigen::Vector4d(pl(0), pl(1), pl(2), d_eff);
  }

  // Scaled+padded vertices (used by the ray/shadow path and shape visualisation).
  m_scaledVertices.resize(m_vertices.size());
  for (size_t i=0; i<m_vertices.size(); i++)
  {
    tf2::Vector3 delta = m_vertices[i] - m_meshCenter;
    double len = delta.length();
    tf2::Vector3 outward = (len > 1e-9) ? (delta / len) : tf2::Vector3(0, 0, 0);
    m_scaledVertices[i] = m_meshCenter + delta * m_scale + outward * m_padding;
  }
}
bool ConvexMesh::isPointInsidePlanes(const tf2::Vector3 &point) const
{
  // Use the scale/padding-adjusted planes so containment honours setScale/setPadding.
  const std::vector<Eigen::Vector4d> &planes = m_paddedPlanes.empty() ? m_planes : m_paddedPlanes;
