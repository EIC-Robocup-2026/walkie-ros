...
public:
  ...
  // Uniform scale/padding support (added). The upstream package left meshes
  // unpadded, so exact-fit collision meshes never caught surface returns. A
  // uniform scale grows the hull multiplicatively about its centre; padding
  // grows it by a fixed distance (metres) along each face normal -- the latter
  // is the right knob to absorb TF/backlash error.
...
protected:
  ...
  std::vector<Eigen::Vector4d> m_paddedPlanes; // m_planes with scale+padding folded into d
