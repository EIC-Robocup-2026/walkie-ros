...
void Dynamics::SetGravity(double gx, double gy, double gz) {
    gravity_x_ = gx;
    gravity_y_ = gy;
    gravity_z_ = gz;
}

bool Dynamics::BuildSolver() {
    if (!kdl_parser::treeFromUrdfModel(*urdf_model_interface, kdl_tree)) {
        fprintf(stderr, "Failed to extract KDL tree\n");
        return false;
    }

    if (!kdl_tree.getChain(start_link, end_link, kdl_chain)) {
        fprintf(stderr, "Failed to get KDL chain from '%s' to '%s'\n", start_link.c_str(),
                end_link.c_str());
        return false;
    }

    std::cout << "[Dynamics] kdl_chain.getNrOfJoints() = " << kdl_chain.getNrOfJoints()
              << std::endl;
    ...
    solver = std::make_unique<KDL::ChainDynParam>(
        kdl_chain, KDL::Vector(gravity_x_, gravity_y_, gravity_z_));
...
bool Dynamics::InitFromString(const std::string &urdf_xml) {
    urdf_model_interface = urdf::parseURDF(urdf_xml);
    if (!urdf_model_interface) {
        fprintf(stderr, "Failed to parse URDF from string\n");
        return false;
    }
    return BuildSolver();
}
