...
backup() { [[ -f "$1" ]] || return 0; cp "$1" "${1}.bak"; warn "Backed up → ${1}.bak"; }
...
EOF
    ...
    #
    # Bind/trigger ONLY on the primary channel's .device unit. Secondary channels
    # on a shared USB interface (e.g. PCAN-USB Pro FD dual-channel, both on the
    # same usbN:1.0) are created together with the primary, but udev does not
    # activate a .device unit for the 2nd+ netdev on that interface — a
    # BindsTo/After on the inactive unit makes the whole oneshot fail with
    # result 'dependency', so nothing comes up. ExecStart still configures every
    # channel by name; they all exist by the time the primary unit activates.
    local primary_alias primary_unit
    primary_alias=$(alias_n "$alias" 0)
    primary_unit="sys-subsystem-net-devices-${primary_alias}.device"
    local after_lines="After=${primary_unit}"$'\n'
    local binds_lines="BindsTo=${primary_unit}"$'\n'
    local wantedby_lines="WantedBy=${primary_unit}"$'\n'

    local chan_list="" exec_lines="" stop_lines=""
    for i in "${!DETECTED_IFACES[@]}"; do
        local chan_alias
        chan_alias=$(alias_n "$alias" "$i")
        chan_list+="${chan_alias} "
        ...
        # The '-' prefixes the COMMAND (after '='), not the key, so systemd
        # ignores a failing pre-down without rejecting the unit file.
...
cmd_uninstall() {
    ...
    if [[ -f "$service_file" ]]; then
        # Channel aliases come from the '# Channels:' header (BindsTo= now lists
        # only the primary). Fall back to BindsTo= for services written by older
        # versions of this script.
        local chan_aliases=()
        local chan_line
        chan_line=$(grep -m1 '^# Channels:' "$service_file" | sed 's/^# Channels: *//' || true)
        if [[ -n "$chan_line" ]]; then
            read -r -a chan_aliases <<< "$chan_line"
        else
...
cmd_status() {
    ...
    if [[ -f "$service_file" ]]; then
        # Channel aliases come from the '# Channels:' header (fall back to
        # BindsTo= for services written by older versions of this script).
        local chan_aliases=()
        local chan_line
        chan_line=$(grep -m1 '^# Channels:' "$service_file" | sed 's/^# Channels: *//' || true)
        if [[ -n "$chan_line" ]]; then
