...
class ObPoseNode(Node):
    def __init__(self):
        ...
        self.create_subscription(
            CameraInfo, "/zed/zed_node/depth/camera_info", self.info_callback, qos
        )
        self.create_subscription(
            Image, "/zed/zed_node/depth/depth_registered", self.depth_callback, qos
        )
        self.create_subscription(
    ...
    def get_absolute_position(self, x_cam, y_cam, z_cam, frame_id):
        ...
        try:
            ...
            self.get_logger().warn(
                f"TF Transform Failed: {e}", throttle_duration_sec=1.0
            )
            return None
    ...
    def yolo_callback(self, msg):
        if not self.is_active or self.latest_depth is None or self.fx is None:
            return
        ...
        for det2d in msg.detections:
            ...
            if (
                ...
                and 0 <= v < self.latest_depth.shape[0]
            ):
                ...
                if z is not None:
                    ...
                    abs_pos = self.get_absolute_position(
                        x_rel, y_rel, z, self.latest_header.frame_id
                    )
                    ...
                    self.get_logger().info(
                        f"Could not find valid depth near x:{u} y:{v}",
