...
Publishes
─────────
  /yolo/tracked_detections_2d          vision_msgs/Detection2DArray
  /yolo/debug_image                    sensor_msgs/Image  (annotated, optional)

How tracking works
──────────────────
ultralytics' .track() returns per-detection track IDs maintained across frames
using ByteTrack by default. We surface those IDs as the ObjectHypothesisWithPose.id
field so OBB3D can reuse them as its database keys.

When tracking is lost (object leaves frame, occlusion, low-confidence streak),
ByteTrack assigns a NEW integer ID. OBB3D handles ID reuse via centroid drift
detection (EC-6.2) so this is safe.

Performance note
────────────────
On the target ROG i9 / RTX 5090 with VLA hogging GPU, run a small model:
  yolov8n.pt   (nano)   — ~3-5 ms per frame
  yolov8s.pt   (small)  — ~6-10 ms per frame
"""
