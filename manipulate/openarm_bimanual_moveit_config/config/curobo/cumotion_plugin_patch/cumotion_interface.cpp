  ...
  planning_interface::MotionPlanDetailedResponse & response)
{
  ...
  // Seed the trajectory's reference state from the scene's CURRENT (full-robot)
  // state, not a zero state: cuMotion's plan_response.trajectory_start carries
  // ONLY the planning group's cspace joints, so a zero-seeded reference would
  // leave every other joint at 0 (lift down -> arms in the base, idle arm folded
  // -> base/self contact). That made move_group's ValidateSolution response
  // adapter FALSE-reject valid cuMotion plans. Seeding from getCurrentState()
  // fills the non-group joints with their real values, so ValidateSolution can
  // mesh-check the arm trajectory against the base/torso (SRDF disables the safe
  // base<->arm pairs) and reject only genuine mid-path collisions. Also renders
  // the trajectory at the correct lift height in RViz.
